...
task :watch, :option do |t, args|
  option = args[:option]
  if option.nil? or option.empty?
    execute("jekyll serve --config _config.yml,_config-dev.yml")
  else
    if option == "drafts"
      execute("jekyll serve --config _config.yml,_config-dev.yml --drafts")
    else
      execute("jekyll serve --config _config.yml,_config-dev.yml --limit_posts #{option}")
    end
  end
end
